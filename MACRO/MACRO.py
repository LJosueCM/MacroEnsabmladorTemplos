...
cont_ALA_i = 0
#Contador que tiene las lineas que se an leido de la macro
cont_lineas = 0
...
def Ver_Macro(linea):
	...
	if encont != -1:
		pos_macro = encont
		#Si ya habia una MACRO pero no un MEND se manda error
		if enaux > 0 and cont_lineas > 2: 
			print("Error: MEND no definido")
			return -1
		#Se incrementa el contador de macros
		enaux += 1
	...
	#Si find regresa un -1 es que no está en la linea
	if encont != -1: 
		#Se le resta a la bandera un 1 de que ya se hizo toda la macro
		enaux -= 1
		#Si es menor a cero se escribieron MEND sin escribir MACROS
		if enaux < 0:
			print("Error: MACROS sin definir")
			return -2
		#Se regresa la opcion de encontrado MEND
		return 2
    
#Función que cambia las referencias de los argumentos
def BusqALA(linea, cont_ALA_i):
	...
	if pos != -1:		#Se corta la cadena hasta donde estan las referencias
		...
		for i in range(cont_ALA_i-1, len(lista_ALA)):
...
def CopiarMacro(linea):
	...
	#Se incrementa pro la siguiente linea
	cont_lineas += 1
	#Verificamos si se encuentra en MEND
	op=Ver_Macro(linea)
	#Se verifica que no haya un error
	if (op == -1 or op == -2) and cont_lineas > 2:
	...
	while op != 2:
		...
		if not linea:
			...
			break
		#Se incrementa por la linea que se acaba de contar
		cont_lineas += 1
...
if len(sys.argv) == 2:
	...
	for linea in archivo_e:
		...
		#Opcion que indica que hubo un error en la funcion Ver_Macro
			...
			sys.exit(0)
		#Para que no se escriba el MEND
		elif op == 2:
			pass
		else:
